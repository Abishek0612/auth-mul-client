...
const GRNWorkspace = () => {
  ...
  const fetchWorkspaceData = useCallback(async () => {
    setIsLoading(true);
    try {
      const response = await workspaceService.getGRNWorkspace({
      ...
      if (response.success) {
        setDocuments(response.data.grns);
  ...
  return (
    <div className="h-full w-full">
      ...
      {isLoading ? (
        ...
        <GRNTable documents={documents} onRowClick={handleRowClick} />
      )}
      ...
      >
        {selectedDocument && (
          <>
            {activeTab === "details" && (
              <GRNDetailsTab data={selectedDocument} />
            )}
            {activeTab === "lines" && <GRNLinesTab items={[]} />}
            {activeTab === "po" && (
