...
const DocumentLibrary = () => {
  ...
  const updateURLParams = useCallback(() => {
    ...
    if (activeSubTab !== "all") params.set("subTab", activeSubTab);
  ...
  const fetchDocuments = useCallback(
    async (page = 1, showRefreshingIndicator = false) => {
      ...
      try {
        const params = {
          ...
          status:
            selectedStatus !== "All Statuses" ? selectedStatus : undefined,
        ...
        if (response.success) {
          ...
          const approvedCount = transformedDocuments.filter(
            (doc) => doc.status === "approved"
          ).length;
          const approvedInvoiceCount = transformedDocuments.filter(
            (doc) =>
              doc.status === "approved" && doc.type.toLowerCase() === "invoice"
          ).length;
          const approvedPOCount = transformedDocuments.filter(
            (doc) =>
              doc.status === "approved" &&
              doc.type.toLowerCase() === "purchase order"
          ).length;
          const approvedGRNCount = transformedDocuments.filter(
            (doc) =>
              doc.status === "approved" && doc.type.toLowerCase() === "grn"
          ).length;
          const approvedPaymentAdviceCount = transformedDocuments.filter(
            (doc) =>
              doc.status === "approved" &&
          ...
          setDocumentCounts({
            ...
            approvedInvoice: approvedInvoiceCount,
            approvedPO: approvedPOCount,
            approvedGRN: approvedGRNCount,
            approvedPaymentAdvice: approvedPaymentAdviceCount,
          });
        }
      } catch {
    ...
    [itemsPerPage, searchTerm, selectedType, selectedStatus, dateRange]
  );

  useEffect(() => {
    fetchDocuments(currentPage);
    updateURLParams();
  }, [fetchDocuments, updateURLParams, currentPage]);
  ...
  const handleTabChange = (tab, subTab = "all") => {
    ...
    setCurrentPage(1);
    setSelectedStatus(tab === "approved" ? "approved" : "All Statuses");
    setSelectedType(
      subTab === "invoice"
        ? "Invoice"
        ...
        <div className="flex justify-between items-center -mb-px">
          <div className="flex space-x-4 sm:space-x-8 overflow-x-auto">
            ...
            <button
              ...
              onClick={() => handleTabChange("approved", "invoice")}
            >
        ...
        <div className="mb-4 sm:mb-6">
          <div className="flex space-x-4 sm:space-x-8 overflow-x-auto border-b border-gray-100">
            <button
              ...
              onClick={() => handleTabChange("approved", "invoice")}
            >
              Invoice ({documentCounts.approvedInvoice})
            </button>
            <button
              className={`py-2 sm:py-3 text-sm font-medium border-b-2 cursor-pointer whitespace-nowrap ${
              ...
              onClick={() => handleTabChange("approved", "purchase-order")}
            >
              Purchase Order ({documentCounts.approvedPO})
            </button>
            <button
              className={`py-2 sm:py-3 text-sm font-medium border-b-2 cursor-pointer whitespace-nowrap ${
                activeSubTab === "grn"
              ...
              onClick={() => handleTabChange("approved", "grn")}
            >
              GRN ({documentCounts.approvedGRN})
            </button>
            <button
              className={`py-2 sm:py-3 text-sm font-medium border-b-2 cursor-pointer whitespace-nowrap ${
              ...
              onClick={() => handleTabChange("approved", "payment-advice")}
            >
              Payment Advice ({documentCounts.approvedPaymentAdvice})
            </button>
          </div>
        </div>
      )}
